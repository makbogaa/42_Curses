...
/*   Updated: 2025/03/20 15:48:14 by makboga          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_square_map(t_win *pnc)
{
	int	i;
	int	row_length;

	if (!pnc->map || !pnc->map->mappin)
	{
		ft_printf("Error: Map is invalid!\n");
		ft_free_game(pnc);
		exit(1);
	}
	row_length = ft_strlen(pnc->map->mappin[0]);
	i = 1;
	while (pnc->map->mappin[i])
	{
		if (ft_strlen(pnc->map->mappin[i]) != row_length)
			return ;
		i++;
	}
	if (i == row_length)
	{
		ft_printf("Error: The map is square!\n");
		ft_free_game(pnc);
		exit(1);
	}
}
