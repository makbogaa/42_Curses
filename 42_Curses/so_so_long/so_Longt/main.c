...
/*   Updated: 2025/03/20 15:47:45 by makboga          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	open_windows(t_win *pnc, char *path)
{
	...
	coin_control(pnc);
	char_control(pnc);
	check_square_map(pnc);
	check_valid_path(pnc);
	is_player_surrounded(pnc);
	is_coin_surrounded(pnc,x,y);
	get_img(pnc, x, y);
}
